...
int currentSequenceNumber = 0;  // it's the next number that the writer will send and that the receiver wants to receive
...
int llread(uint8_t* packet) {
    DataMachine* dm = data_machine_init(MAX_DATA_SIZE);
    uint8_t byte;
